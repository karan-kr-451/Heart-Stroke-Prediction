...
from src.StrokeProject.utils.util import load_object
...
class HeartStrokeData:
    ...
    def get_Strokeproject_input_data_frame(self):

        try:
            Strokeproject_input_dict = self.get_Strokeproject_data_as_dict()
            return pd.DataFrame(Strokeproject_input_dict)
        except Exception as e:
            raise HeartStrokeException(e, sys) from e

    def get_Strokeproject_data_as_dict(self):
        try:
            input_data = {
                "gender": [self.gender],
                "age": [self.age],
